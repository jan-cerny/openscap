 ...
 * names.
 */
OSCAP_EXPORT fsdev_t *fsdev_init(const char **fs, size_t fs_cnt);

/**
 * Initialize the fsdev_t structure from a string containing filesystem
 * names.
 */
OSCAP_EXPORT fsdev_t *fsdev_strinit(const char *fs_names);

/**
 * Free the fsdev_t structure.
 */
OSCAP_EXPORT void fsdev_free(fsdev_t * lfs);

/**
 * Search an id in the fsdev_t structure.
 */
OSCAP_EXPORT int fsdev_search(fsdev_t * lfs, void *id);
 ...
 * @retval -1 error
 */
OSCAP_EXPORT int fsdev_path(fsdev_t * lfs, const char *path);
 ...
 * @retval -1 error
 */
OSCAP_EXPORT int fsdev_fd(fsdev_t * lfs, int fd);
