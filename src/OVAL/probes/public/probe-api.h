 ...
 */
OSCAP_EXPORT SEXP_t *probe_item_build(const char *fmt, ...);

OSCAP_EXPORT /* SEXP_t *probe_item_creat (const char *name, SEXP_t *attrs, ...); */
/**
 ...
 */
OSCAP_EXPORT SEXP_t *probe_item_new(const char *name, SEXP_t * attrs);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_item_attr_add(SEXP_t * item, const char *name, SEXP_t * val);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_item_ent_add(SEXP_t * item, const char *name, SEXP_t * attrs, SEXP_t * val);
 ...
 */
OSCAP_EXPORT int probe_item_setstatus(SEXP_t * obj, oval_syschar_status_t status);
 ...
 * @param status the new status
 */
OSCAP_EXPORT int probe_itement_setstatus(SEXP_t * obj, const char *name, uint32_t n, oval_syschar_status_t status);
 ...
 * @return a new id
 */
OSCAP_EXPORT SEXP_t *probe_item_newid(struct id_desc_t *id_desc);

/**
 * Reset the item id generator.
 * @param id_desc pointer to a structure holding the global id context
 */
OSCAP_EXPORT void probe_item_resetidctr(struct id_desc_t *id_desc);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_attr_creat(const char *name, const SEXP_t * val, ...);
 ...
 * @param fmt the desired format
 */
OSCAP_EXPORT SEXP_t *probe_obj_build(const char *fmt, ...);
 ...
 * @param ... arbitrary number of entity arguments
 */
OSCAP_EXPORT SEXP_t *probe_obj_creat(const char *name, SEXP_t * attrs, ...);
 ...
 * @param attrs optional attributes argument
 */
OSCAP_EXPORT SEXP_t *probe_obj_new(const char *name, SEXP_t * attrs);
 ...
 * @param n select the n-th occurence of an entity with the specified name
 */
OSCAP_EXPORT SEXP_t *probe_obj_getent(const SEXP_t * obj, const char *name, uint32_t n);
 ...
 * @param n select the n-th occurence of an entity with the specified name
 */
OSCAP_EXPORT SEXP_t *probe_obj_getentval(const SEXP_t * obj, const char *name, uint32_t n);
 ...
 */
OSCAP_EXPORT int probe_obj_getentvals(const SEXP_t * obj, const char *name, uint32_t n, SEXP_t ** res);
 ...
 * @param name the name of the attribute
 */
OSCAP_EXPORT SEXP_t *probe_obj_getattrval(const SEXP_t * obj, const char *name);
 ...
 * @param name the name of the attribute
 */
OSCAP_EXPORT bool probe_obj_attrexists(const SEXP_t * obj, const char *name);
 ...
 * @param status the new status
 */
OSCAP_EXPORT int probe_obj_setstatus(SEXP_t * obj, oval_syschar_status_t status);
 ...
 * @param status the new status
 */
OSCAP_EXPORT int probe_objent_setstatus(SEXP_t * obj, const char *name, uint32_t n, oval_syschar_status_t status);

/**
 * Get the name of an object.
 * @param obj the queried object
 */
OSCAP_EXPORT char *probe_obj_getname(const SEXP_t * obj);
 ...
 */
OSCAP_EXPORT size_t probe_obj_getname_r(const SEXP_t * obj, char *buffer, size_t buflen);

/*
 * collected objects
 */

OSCAP_EXPORT SEXP_t *probe_cobj_new(oval_syschar_collection_flag_t flag, SEXP_t *msg_list, SEXP_t *item_list, SEXP_t *mask_list);
OSCAP_EXPORT int probe_cobj_add_msg(SEXP_t *cobj, const SEXP_t *msg);
OSCAP_EXPORT SEXP_t *probe_cobj_get_msgs(const SEXP_t *cobj);
OSCAP_EXPORT SEXP_t *probe_cobj_get_mask(const SEXP_t *cobj);
OSCAP_EXPORT int probe_cobj_add_item(SEXP_t *cobj, const SEXP_t *item);
OSCAP_EXPORT SEXP_t *probe_cobj_get_items(const SEXP_t *cobj);
OSCAP_EXPORT void probe_cobj_set_flag(SEXP_t *cobj, oval_syschar_collection_flag_t flag);
OSCAP_EXPORT oval_syschar_collection_flag_t probe_cobj_get_flag(const SEXP_t *cobj);
...
OSCAP_EXPORT oval_syschar_collection_flag_t probe_cobj_compute_flag(SEXP_t *cobj);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_msg_creat(oval_message_level_t level, char *message);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_msg_creatf(oval_message_level_t level, const char *fmt, ...) __attribute__((format(printf, 2, 3), nonnull(2)));
 ...
 */
OSCAP_EXPORT SEXP_t *probe_ent_creat(const char *name, SEXP_t * attrs, SEXP_t * val, ...);
 ...
 * @param val the value of the entity
 */
OSCAP_EXPORT SEXP_t *probe_ent_creat1(const char *name, SEXP_t * attrs, SEXP_t * val);
 ...
 * @param val value of the new attribute
 */
OSCAP_EXPORT SEXP_t *probe_ent_attr_add(SEXP_t * ent, const char *name, SEXP_t * val);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_ent_getval(const SEXP_t * ent);
 ...
 * @return number of values in the list stored in the res argument
 */
OSCAP_EXPORT int probe_ent_getvals(const SEXP_t * ent, SEXP_t ** res);
 ...
 * @param name the name of the attribute
 */
OSCAP_EXPORT SEXP_t *probe_ent_getattrval(const SEXP_t * ent, const char *name);
 ...
 * @param name the name of the attribute
 */
OSCAP_EXPORT bool probe_ent_attrexists(const SEXP_t * ent, const char *name);
 ...
 */
OSCAP_EXPORT int probe_ent_setdatatype(SEXP_t * ent, oval_datatype_t type);

/**
 * Get the OVAL data type of an entity.
 * @param ent the queried entity
 */
OSCAP_EXPORT oval_datatype_t probe_ent_getdatatype(const SEXP_t * ent);
 ...
 * @mask the new mask
 */
OSCAP_EXPORT int probe_ent_setmask(SEXP_t * ent, bool mask);

/**
 * Get entity's mask.
 * @param ent the queried entity
 */
OSCAP_EXPORT bool probe_ent_getmask(const SEXP_t * ent);
 ...
 * @param status the new status
 */
OSCAP_EXPORT int probe_ent_setstatus(SEXP_t * ent, oval_syschar_status_t status);

/**
 * Get entity status.
 * @param ent the queried entity
 */
OSCAP_EXPORT oval_syschar_status_t probe_ent_getstatus(const SEXP_t * ent);

/**
 * Get the name of an entity.
 * @param ent the queried entity
 */
OSCAP_EXPORT char *probe_ent_getname(const SEXP_t * ent);
 ...
 * @param buflen the length of the buffer
 */
OSCAP_EXPORT size_t probe_ent_getname_r(const SEXP_t * ent, char *buffer, size_t buflen);

/**
 * Free the memory allocated by the probe_* functions.
 * @param obj the object to be freed
 */
OSCAP_EXPORT void probe_free(SEXP_t * obj);
 ...
 */
OSCAP_EXPORT void probe_filebehaviors_canonicalize(SEXP_t **behaviors);
 ...
 * @param behaviors address of the pointer to the 'behaviors' entity, must not be NULL
 */
OSCAP_EXPORT void probe_tfc54behaviors_canonicalize(SEXP_t **behaviors);
...
OSCAP_EXPORT void probe_offline_mode(void);
OSCAP_EXPORT void probe_preload(void);
OSCAP_EXPORT void *probe_init(void) __attribute__ ((unused));
OSCAP_EXPORT void probe_fini(void *) __attribute__ ((unused));

typedef struct probe_ctx probe_ctx;

OSCAP_EXPORT int probe_main(probe_ctx *, void *) __attribute__ ((nonnull(1)));

OSCAP_EXPORT bool probe_item_filtered(const SEXP_t *item, const SEXP_t *filters);

OSCAP_EXPORT int probe_result_additem(SEXP_t *result, SEXP_t *item);
 ...
 */
OSCAP_EXPORT int probe_item_collect(probe_ctx *ctx, SEXP_t *item);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_ctx_getobject(probe_ctx *ctx);
 ...
 */
OSCAP_EXPORT SEXP_t *probe_ctx_getresult(probe_ctx *ctx);
...
OSCAP_EXPORT SEXP_t *probe_item_create(oval_subtype_t item_subtype, probe_elmatr_t *item_attributes[], ...);

#define PROBE_ENT_AREF(ent, dst, attr_name, invalid_exp)		\
	do {								\
		if (((dst) = probe_ent_getattrval(ent, attr_name)) == NULL) { \
OSCAP_EXPORT 			dE("Attribute `%s' is missing!", attr_name);	\
			invalid_exp					\
		}               					\
	} while(0)
...
OSCAP_EXPORT 			dW("Entity has no value!");		\
			invalid_exp				\
		} else {					\
			if (!SEXP_stringp(___r)) {		\
OSCAP_EXPORT 				dE("Invalid type");		\
OSCAP_EXPORT 				SEXP_free(___r);		\
				invalid_exp			\
			}					\
...
OSCAP_EXPORT 				SEXP_string_cstr_r(___r, dst, dstlen); \
OSCAP_EXPORT 				SEXP_free(___r);		\
			}					\
		}						\
	} while (0)
...
OSCAP_EXPORT 				dst = SEXP_number_geti_32(___r);	\
OSCAP_EXPORT 				SEXP_free(___r);                	\
			}						\
		}							\
	} while (0)
...
OSCAP_EXPORT oval_operation_t probe_ent_getoperation(SEXP_t *entity, oval_operation_t op);

OSCAP_EXPORT int probe_item_add_msg(SEXP_t *item, oval_message_level_t msglvl, char *msgfmt, ...);

OSCAP_EXPORT SEXP_t *probe_entval_from_cstr(oval_datatype_t type, const char *value, size_t vallen);
OSCAP_EXPORT SEXP_t *probe_ent_from_cstr(const char *name, oval_datatype_t type, const char *value, size_t vallen);

OSCAP_EXPORT OSCAP_DEPRECATED(oval_version_t probe_obj_get_schema_version(const SEXP_t *obj));
OSCAP_EXPORT oval_schema_version_t probe_obj_get_platform_schema_version(const SEXP_t *obj);

/**
 * Get object entity mask
 * @returns a list of masked entities
 */
OSCAP_EXPORT SEXP_t *probe_obj_getmask(SEXP_t *obj);
