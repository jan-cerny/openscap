 ...
 * \par
 * \code
OSCAP_EXPORT  *  struct oval_definition_model * def_model = oval_definition_model_import(oval_file);
OSCAP_EXPORT  *  struct xccdf_benchmark * benchmark = xccdf_benchmark_import(file);
OSCAP_EXPORT  *  struct xccdf_policy_model * policy_model = xccdf_policy_model_new(benchmark);
OSCAP_EXPORT  *  struct oval_agent_session * sess = oval_agent_new_session(def_model, "name-of-file");
 *  ...
OSCAP_EXPORT  *  xccdf_policy_model_register_engine_and_query_callback(policy_model, "http://oval.mitre.org/XMLSchema/oval-definitions-5", oval_agent_eval_rule, (void *) sess, NULL);
 * \endcode
 * 
 */
 ...
 */
OSCAP_EXPORT int oval_agent_resolve_variables(struct oval_agent_session * session, struct xccdf_value_binding_iterator *it);
 ...
 */
OSCAP_EXPORT bool xccdf_policy_model_register_engine_oval(struct xccdf_policy_model * model, struct oval_agent_session * sess);
 ...
 */
OSCAP_EXPORT OSCAP_DEPRECATED(void oval_agent_export_sysinfo_to_xccdf_result(struct oval_agent_session * session, struct xccdf_result * ritem));
