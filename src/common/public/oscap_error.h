 ...
 *
 * @code
OSCAP_EXPORT  * syschar = oval_probe_object_eval (pctx, object);
 * if (syschar == NULL && oscap_err()) {
OSCAP_EXPORT  *     printf("Error: (%d) %s\n", oscap_err_family(), oscap_err_desc());
 * }
 * oscap_clearerr()
 * @endcode
 *
 ...
 * Clear an error.
 */
OSCAP_EXPORT void oscap_clearerr(void);

/**
 * Check for an error.
 */
OSCAP_EXPORT bool oscap_err(void);

/**
 * Get last error family.
 */
OSCAP_EXPORT oscap_errfamily_t oscap_err_family(void);

/**
 * Get last error description.
 */
OSCAP_EXPORT const char *oscap_err_desc(void);
 ...
 */
OSCAP_EXPORT char *oscap_err_get_full_error(void);
